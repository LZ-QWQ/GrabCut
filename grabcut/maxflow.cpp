	...
	flowtype Graph<captype,tcaptype,flowtype>::maxflow(bool reuse_trees, Block<node_id>* _changed_list)
{
	...
	//ÎÞÄÎÖ®¾Ù
	char temp1[] = "reuse_trees cannot be used in the first call to maxflow()!";
	char temp2[] = "changed_list cannot be used without reuse_trees!";
	if (maxflow_iteration == 0 && reuse_trees) { if (error_function) (*error_function)(temp1); exit(1); }
	if (changed_list && !reuse_trees) { if (error_function) (*error_function)(temp2); exit(1); }
